...
const baseDir = 'e:/EndoscopeServices/Web/AI WEB ES';
const indexPath = path.join(baseDir, 'index.html');
const indexHtml = fs.readFileSync(indexPath, 'utf-8');

// Extract header and footer
const headerMatch = indexHtml.match(/<!DOCTYPE html>[\s\S]*?<\/header>/);
...
const pages = [
    {
        ...
        <div class="two-col-grid">
            ...
            <a href="rigid-scope-repair.html" class="content-card">
                <h3>🔭 Rigid Scope Repair</h3>
                <p>Precision repair of rigid endoscopes for clear, reliable imaging.</p>
            </a>
        </div>
        <div class="info-box">
    ...
    {
        fileName: 'rigid-scope-repair.html',
        title: 'Rigid Scope Repair',
        desc: 'Precision repair of rigid endoscopes for clear, reliable imaging.',
        content: `
        <h2>Flawless Rigid Endoscopy Repairs</h2>
        <p>We restore rod lens systems to perfect clarity. Our optical engineers specialize in re-establishing the pristine image quality expected from your rigid scopes.</p>
        <div class="two-col-grid">
            <div class="content-card">
                <h3>Optical Restoration</h3>
                <ul>
                    <li>Objective lens replacement</li>
                    <li>Rod lens replacement and realigning</li>
                    <li>Eyepiece rebuilding</li>
                    <li>Light fiber replacement</li>
                </ul>
            </div>
            <div class="content-card">
                <h3>Laser Welding</h3>
                <p>We utilize precision laser welding to hermetically seal the scopes, preventing any future fluid invasion and ensuring sterilization integrity.</p>
            </div>
        </div>
    `
    },
                ...
                <form id="contact-form">
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="facility" required placeholder="General Hospital">
                        </div>
                    </div>
                    <div class="form-group">
...
pages.forEach(page => {
    ...
    let fullHtml = header.replace(/<title>.*?<\/title>/, \`<title>\${page.title} | Endoscope Services</title>\`);
    ...
    fullHtml = fullHtml.replace(/class="nav-link active"/g, 'class="nav-link"');
    
    if (page.fileName === 'services.html' || page.fileName.includes('-repair.html') || page.fileName === 'surgical-power-tools.html') {
        fullHtml = fullHtml.replace('href="services.html" class="nav-link"', 'href="services.html" class="nav-link active"');
    } else if (page.fileName === 'sell-equipment.html') {
        fullHtml = fullHtml.replace('href="sell-equipment.html" class="nav-link"', 'href="sell-equipment.html" class="nav-link active"');
    } else if (page.fileName === 'handling-procedures.html') {
    ...
    } else if (page.fileName === 'contact.html' || page.fileName.includes('loaner-tc.html') || page.fileName.includes('warranty.html') || page.fileName.includes('loan-endoscopes.html')) {
...
\${page.content}
    ...
    fullHtml += heroSection + contentSection + footer;

    fs.writeFileSync(path.join(baseDir, page.fileName), fullHtml, 'utf-8');
});

// Generate Sitemap.xml
const baseUrl = 'https://endoscopeservices.com.au';
...
pages.forEach(page => {
    const priority = page.fileName === 'index.html' ? '1.00' : '0.80';
    sitemap += `  <url>\n    <loc>${baseUrl}/${page.fileName}</loc>\n    <priority>${priority}</priority>\n  </url>\n`;
});

sitemap += '</urlset>';
fs.writeFileSync(path.join(baseDir, 'sitemap.xml'), sitemap, 'utf-8');

console.log('Pages and Sitemap generated successfully!');
